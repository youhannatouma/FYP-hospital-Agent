  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between px-2 pt-4">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground mt-4 font-medium text-lg max-w-lg leading-relaxed">
            Manage your longitudinal patient interactions. Streamline your workflow with real-time session synchronization.
          </p>
        </div>
        <Button 
          size="lg"
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 px-2">
         {[
           { label: "Active Sessions", value: upcoming.length, icon: CalendarDays, color: "text-blue-500", bg: "bg-blue-500/10" },
           { label: "Concluded", value: appointmentList.filter(a => a.status === "Completed").length, icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-500/10" },
           { label: "Decommissioned", value: appointmentList.filter(a => a.status === "Cancelled").length, icon: XCircle, color: "text-destructive", bg: "bg-destructive/10" }
         ].map((stat, i) => (
      ...
      <Tabs defaultValue="upcoming" className="w-full">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 px-2">
          ...
          <Button variant="outline" size="sm" className="h-11 rounded-xl border-border/50 px-4 font-black text-[10px] uppercase tracking-widest">
        ...
        <AnimatePresence mode="wait">
          {[{ key: "upcoming", items: upcoming }, { key: "past", items: past }].map(({ key, items }) => (
            <TabsContent key={key} value={key} className="mt-4 flex flex-col gap-6 px-2 outline-none">
              {items.map((appt, idx) => (
                <m.div
                  key={appt.id}
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center gap-8">
                        ...
                        <div className="flex items-center gap-6 lg:w-1/3">
                          ...
                          <div className="min-w-0">
                            <Badge className={cn("mb-2 border-none text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg", 
                              appt.status === 'Upcoming' ? 'bg-blue-500/10 text-blue-500' : 
                            ...
                            <p className="text-xs font-bold text-muted-foreground mt-2 uppercase tracking-wider">{appt.type}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-6 py-6 lg:py-0 border-y lg:border-y-0 lg:border-x border-border/30 lg:px-8">
                          <div className="space-y-1.5">
                            ...
                            <p className="text-sm font-black text-foreground">{appt.date}</p>
                          </div>
                          <div className="space-y-1.5">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none flex items-center gap-2">
                               <Clock className="h-3 w-3" /> Time Window
                            </span>
                            <p className="text-sm font-black text-foreground">{appt.time} <span className="text-[10px] text-muted-foreground ml-1">({appt.duration})</span></p>
                          </div>
                          <div className="space-y-1.5">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none flex items-center gap-2">
                               {appt.isVideo ? <Video className="h-3 w-3" /> : <MapPin className="h-3 w-3" />} Protocol
                            </span>
                            <p className="text-sm font-black text-foreground">{appt.isVideo ? 'Digital' : 'In-Person'}</p>
                          </div>
                        </div>
                            ...
                            <>
                              {appt.isVideo && (
                                <Button
                                  ...
                                  onClick={() => handleJoinCall(appt.patientName)}
                                >
                              ...
                              <Button
                                size="lg"
                                variant="outline"
                                className="h-12 px-6 rounded-xl border-border/50 text-foreground font-black text-[10px] uppercase tracking-widest hover:bg-muted/50 transition-all gap-2"
                      ...
                      {appt.notes && (
                        <div className="mt-8 flex items-start gap-4 p-5 rounded-[1.5rem] bg-muted/30 border border-dashed border-border/50">
                           ...
                           <div className="space-y-1">
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-50">Clinical Agenda</p>
      ...
      <RecordDetailDialog 
        ...
        onEdit={(record) => {
          ...
          toast({ title: "Modifying Subject", description: `Loading clinical editor for ${record.name}.` })
        }}
        onDownload={(record) => {
          toast({ title: "Dossier Export", description: `Securely generating clinical summary for ${record.name}.` })
        }}
      />
