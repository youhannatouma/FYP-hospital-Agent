    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => { const { toast } = require('@/hooks/use-toast'); toast({ title: 'Stock Reports', description: 'Generating stock report...' }) }}>
            <Warehouse className="mr-2 h-4 w-4" />
            Stock Reports
          </Button>
          <Button className="bg-primary text-primary-foreground" onClick={() => { const { toast } = require('@/hooks/use-toast'); toast({ title: 'Add Medicine', description: 'Medicine creation form coming soon.' }) }}>
            ...
            <CardContent>
              <div className="space-y-2">
                {MOCK_MEDICINES.map((med) => (
                  <div key={med.name} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/10 hover:border-primary/20 hover:bg-muted/30 transition-all">
                    <div className="flex items-center gap-4">
