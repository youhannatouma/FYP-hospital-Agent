    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => toast({ title: "Export Started", description: "Downloading audit logs CSV..." })}>
            <Download className="mr-2 h-4 w-4" />
            Export Audit Logs
          </Button>
          <Button className="bg-primary text-primary-foreground" onClick={() => toast({ title: "Compliance Check Initiated", description: "Running automated system policy review..." })}>
      ...
      <Tabs defaultValue="audit" className="space-y-6">
        <TabsList className="bg-muted/50 p-1 border border-border/50">
          ...
          <TabsTrigger value="privacy" className="gap-2"><Shield className="h-4 w-4" />Data Privacy</TabsTrigger>
              ...
              <CardContent className="p-0">
                <div className="bg-primary/10 p-6 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <Button size="sm" className="mt-4" onClick={() => toast({ title: "Access Updated", description: "Data access controls refreshed successfully." })}>Update Access Logs</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {MOCK_SECURITY_EVENTS.map((event) => (
            <Card key={event.id} className="border-border/50 bg-card/50 overflow-hidden">
              ...
              <CardContent className="p-4 flex items-start justify-between">
                ...
                <Button size="sm" variant="outline" onClick={() => toast({ title: "Investigation Started", description: `Opening security incident response protocol for ${event.id}...` })}>Investigate</Button>
              </CardContent>
            </Card>
          ))}
        </TabsContent>
