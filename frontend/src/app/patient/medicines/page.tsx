...
export default function MedicinesPage() {
    ...
    const fetchPharmacies = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/patient/pharmacies`,
        );
        if (response.ok) {
  ...
  return (
    <m.div className="flex flex-col gap-10 max-w-[1200px] mx-auto pb-24">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between px-2 pt-4">
        <div className="space-y-4">
          <Badge
            variant="outline"
            className="border-blue-500/20 text-blue-500 bg-blue-500/5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest leading-none"
          >
          ...
          <p className="text-muted-foreground mt-4 font-medium text-lg max-w-lg leading-relaxed">
            Curated medication protocols. Maintain strict adherence for optimal
      ...
      <Tabs defaultValue="prescriptions" className="w-full">
        ...
        <AnimatePresence mode="wait">
          <TabsContent
            value="prescriptions"
            className="flex flex-col gap-8 px-2 outline-none"
          >
          ...
          <TabsContent
            value="history"
            className="flex flex-col gap-6 px-2 outline-none"
          >
            {pastPrescriptions.map((rx) => (
              <Card
                key={rx.id}
                className="premium-card rounded-[2rem] border-none bg-card/50 p-6 flex items-center gap-6 grayscale hover:grayscale-0 transition-all duration-500"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant="secondary"
                      className="text-[8px] font-black uppercase tracking-widest rounded-lg"
                    >
                      {rx.status}
                    </Badge>
                  </div>
                  <p className="text-xs font-bold text-muted-foreground mt-1 opacity-70">
                    Completed Protocol • Prescribed by {rx.doctor}
                  </p>
                </div>
              ...
              <CardContent className="relative z-10 p-10 flex flex-col md:flex-row items-center justify-between gap-10">
                <div className="space-y-4 text-center md:text-left md:max-w-md">
                  ...
                  <p className="text-slate-400 text-sm font-medium leading-relaxed">
                    Upload your physical prescription dossier. Our engine will
                    OCR the data and source availability across regional
                    pharmacy nodes.
                  </p>
                  <Button
                    size="lg"
                    className="bg-white text-slate-900 hover:bg-slate-100 h-14 px-10 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {pharmacyResults.map((pharmacy) => (
                  <Card
                    key={pharmacy.id}
                    className="premium-card rounded-[2rem] border-none bg-card shadow-premium p-8 group"
                  >
                    ...
                    <div className="flex gap-3">
                      ...
                      <Button
                        variant="outline"
                        className="flex-1 h-12 rounded-xl border-border/50 font-black text-[10px] uppercase tracking-widest hover:bg-muted/50 transition-all"
                      >
                        Lock Price
                      </Button>
                    </div>
      ...
      <div className="mt-8 mx-2 bg-muted/20 p-6 rounded-3xl border border-dashed border-border/50 flex items-start gap-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground font-medium leading-relaxed max-w-3xl">
            Medication adherence is the primary factor in longitudinal health
            success. Use the specialized refill protocol 72 hours prior to
            supply depletion to ensure zero-latency therapy continuation.
          </p>
        </div>
      </div>
