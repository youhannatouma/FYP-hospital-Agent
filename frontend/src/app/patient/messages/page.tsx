...
function getCounterpartyName(message: Message, currentUserId: string | undefined): string {
  return message.sender_id === currentUserId
    ? message.receiver_name || "Doctor"
    : message.sender_name || "Doctor"
}
...
export default function MessagesPage() {
  ...
  const loadMessages = React.useCallback(async () => {
    try {
      ...
      console.error("[PatientMessagesPage] Failed to load messages:", error)
      setMessages([])
    } finally {
      setIsLoading(false)
    }
  ...
  const handleOpenMessage = React.useCallback(
    async (message: Message) => {
      ...
      try {
        ...
        console.error("[PatientMessagesPage] Failed to mark message as read:", error)
      }
    },
    [profile?.user_id]
  )
